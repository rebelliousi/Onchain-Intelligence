...
export default function AuraDashboard() {
  ...
  const STATS_CARDS = [
    { 
      ...
      value: stats?.volume || "$2.4B", 
      trend: "+2.4%", 
      color: "var(--color-teal)", 
      icon: "📊" 
    },
    { 
      label: "DeFi Pulse Gainer", 
      value: stats?.topGainer || "Scanning...", 
      trend: "Mooning", 
      color: "var(--color-teal)", 
      icon: "🚀" 
    },
    { 
      label: "Aether Latency", 
      value: stats?.latency || "14.2ms", 
      trend: "Stable", 
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from(".stat-card", { 
        y: 40, opacity: 0, duration: 0.8, stagger: 0.1, ease: "back.out(1.7)", delay: 0.2 
      });
  ...
  return (
    <div className="aura-wrapper" ref={containerRef}>
      <style jsx global>{`
        ...
        .stat-value { font-size: 1.8rem; font-weight: 700; margin: 10px 0; color: #fff; }
      ...
      <main className="container">
        <header className="header-content">
          <h1 style={{ fontSize: '2.5rem', fontWeight: 800 }}>Intelligence Overview</h1>
          <p style={{ color: 'var(--color-gray)', marginTop: '8px' }}>
            The AURA Neural Network is currently <span style={{ color: 'var(--color-teal)' }}>Operational</span>.
          </p>
        </header>
        ...
        <section className="glass chart-container">
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
            <h3 style={{ fontWeight: 800, letterSpacing: '1px' }}>Neural Throughput</h3>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
               <div style={{ width: '6px', height: '6px', background: 'var(--color-teal)', borderRadius: '50%', animate: 'pulse 2s infinite' }}></div>
               <span style={{ color: 'var(--color-teal)', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '2px' }}>LIVE PULSE</span>
            </div>
          </div>
          ...
          <ResponsiveContainer width="100%" height="85%">
            <AreaChart data={stats?.chartData || []}>
              ...
              <Tooltip 
                contentStyle={{ backgroundColor: 'rgba(5, 5, 5, 0.95)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', backdropFilter: 'blur(10px)' }}
        ...
        <section className="glass content-box">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: '0.6rem', color: 'var(--color-gray)', letterSpacing: '2px' }}>TOP PROPHETS (24H)</span>
          </div>
          ...
          <table className="table">
            ...
            <tbody>
              {tableLoading ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: '60px', opacity: 0.3, letterSpacing: '4px' }}>SYNCHRONIZING FEED...</td></tr>
              ) : (
                smartMoney?.map((trader) => (
                  <tr key={trader.rank}>
                    ...
                    <td style={{ fontWeight: 600, color: '#fff' }}>{trader.profit}</td>
                    <td style={{ color: 'var(--color-purple)', fontWeight: 600 }}>{trader.winRate}</td>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <div style={{ flex: 1, height: '4px', background: 'rgba(255,255,255,0.05)', borderRadius: '10px', overflow: 'hidden' }}>
                          <motion.div 
                            initial={{ width: 0 }}
                            animate={{ width: `${trader.load}%` }}
                            transition={{ duration: 1.5, ease: "easeOut" }}
                            style={{ height: '100%', background: 'var(--color-teal)' }} 
                          />
                        </div>
                        <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)', width: '30px' }}>{trader.load}%</span>
                      </div>
                    </td>
                  </tr>
                ))
